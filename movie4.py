...
class MovieStim4(BaseVisualStim, ContainerMixin, TextureMixin):
            ...
            if (not self.noAudio) and (self._mov.audio is not None):
                ...
                try:
                    self._audioStream = sound.Sound(
                        self._mov.audio.to_soundarray(), sampleRate=self._mov.audio.fps
                    )
                except:
                    ...
                    self._audioStream = sound.Sound(
                        jwe_tmp.audio.to_soundarray(), sampleRate=self._mov.audio.fps
                    )
                    del jwe_tmp
    ...
    def play(self, log=True):
        ...
        if status != PLAYING:
            ...
            if (
                self._audioStream is not None
                and self._audioStream.status is not PLAYING
            ):
                self._audioStream.play()
            if status == PAUSED:
                if (
                    self.getCurrentFrameTime() < 0
                ):  # Check for valid timestamp, correct if needed -JK
                    self._audioSeek(0)
                else:
    ...
    def pause(self, log=True):
        ...
        if log and self.autoLog:
            self.win.logOnFlip(
                "Failed Set %s paused" % (self.name), level=logging.EXP, obj=self
            )
        return False
    ...
    def _updateFrameTexture(self):
        ...
        if self._nextFrameT > self.duration:
            ...
            if self._nextFrameT > (
                self._videoClock.getTime() - self._retraceInterval / 2.0
            ):
                return None
        ...
        try:
            ...
            if self.autoLog:
                logging.warning(
                    "Frame {} not found, moving one frame and trying again".format(
                        self._nextFrameT
                    ),
                    obj=self,
                )
        ...
        )  # ?? do we need this - think not!

        if self.status == PLAYING:
            self._nextFrameT += self._frameInterval * 2
